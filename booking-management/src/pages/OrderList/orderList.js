...
const OrderList = () => {
    ...
    const handleUpdateOrder = async (values) => {
        ...
        try {
            ...
            await axiosClient.put("/tours/book/" + id, bookTours).then(response => {
                if (response === undefined) {
                    notification["error"]({
                        message: `Thông báo`,
                        description:
    ...
    const handleCategoryList = async () => {
        try {
            ...
            if(userParse.role === "isCompany"){
                await orderApi.getListBookTourByCompany().then((res) => {
                    console.log(res);
                    setTotalList(res)
                    setOrder(res);
                    setLoading(false);
                ...
                await orderApi.getListBookTour().then((res) => {
                    console.log(res);
                    setTotalList(res)
                    setOrder(res);
                    setLoading(false);
    ...
    const handleEditOrder = (id) => {
        ...
        (async () => {
            try {
                const response = await orderApi.getDetailsBookingTour(id);
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (slugs) => (
                <span >
                    {slugs === "rejected" ? <Tag style={{ width: 120, textAlign: "center" }} color="red">Đã hủy</Tag> : slugs === "approved" ? <Tag style={{ width: 120, textAlign: "center" }} color="geekblue" key={slugs}>
                       Đã xác nhận
                    </Tag> : slugs === "final" ? <Tag color="green" style={{ width: 120, textAlign: "center" }}>Đã hoàn thành tour</Tag> : <Tag color="blue" style={{ width: 120, textAlign: "center" }}>Đợi xác nhận</Tag>}
                </span>
            ),
        },
    ];
